...
public class WeightGraph {
	...
	void printAllShortestDistances(String vert) throws Exception {
		...
		//Construct GraphAlgorithms object
		GraphAlgorithms ga = new GraphAlgorithms();
		
		// Compute shortest-path map
		Map<Vertex<String>, Integer> shortest = ga.shortestPathLengths(sGraph, vSource);
		

        // Print shortest path to named cities
		for (Vertex<String> v : sGraph.vertices()){
			String city = v.getElement();
			if (city.length() > 2){ // Only cities with name length > 2
				Integer dist = shortest.get(v);
				System.out.println(vert + " to " + city + " = " + dist);
			}
		}
