...
public class SimpleGraph {
  ...
  private void DFS(Graph<String,String> graph, Vertex<String> v ) {
    ...
    /***** please implement depth-first search here ************/
	  // If starting vertex is null, nothing to do
	if (v == null) return;

	String key = v.getElement();

	// If already visited, return (prevents infinite loops on cycles)
	Boolean already = visited.get(key);
	if (already != null && already) return;

	// Mark as visited and call the "first visit" hook
	visited.put(key, true);
	startVisit(v);

	// Recurse on every adjacent vertex reachable by outgoing edges
	for (Edge<String> e : graph.outgoingEdges(v)) {
		Vertex<String> w = graph.opposite(v, e);
		String wkey = w.getElement();
		Boolean wvisited = visited.get(wkey);
		if (wvisited == null || !wvisited) {
			DFS(graph, w);
		}
	}
